...
export async function PATCH(request: Request, { params }: RouteCtx) {
  ...
  if (typeof body.title === "string") {
    ...
    if (t.length < 3) return NextResponse.json({ error: "Title must be at least 3 chars" }, { status: 400 });
    update.title = t;
  }
  if (typeof body.overview === "string") {
    const o = body.overview.trim();
    if (o.length < 10) return NextResponse.json({ error: "Overview must be at least 10 chars" }, { status: 400 });
    update.overview = o;
  }
  if (typeof body.stage === "string") {
    if (!ALLOWED_STAGES.has(body.stage)) return NextResponse.json({ error: "Invalid stage" }, { status: 400 });
    update.stage = body.stage;
  }
  if (typeof body.state === "string") {
    if (!ALLOWED_STATES.has(body.state)) return NextResponse.json({ error: "Invalid state" }, { status: 400 });
    update.state = body.state;
  }
  if ("hero_media_id" in body) update.hero_media_id = body.hero_media_id ?? null;
  if ("published_at" in body) {
    if (body.published_at === true) update.published_at = new Date().toISOString();
    else if (body.published_at === false || body.published_at === null) update.published_at = null;
    else if (typeof body.published_at === "string") {
      const d = new Date(body.published_at);
      if (Number.isNaN(d.getTime())) return NextResponse.json({ error: "Invalid published_at" }, { status: 400 });
  ...
  const reason = "published_at" in update ? "publish toggle" : `edit ${cfg.slug} post`;
...
export async function DELETE(_request: Request, { params }: RouteCtx) {
  ...
  const supabase = createSupabaseServiceWithActor(admin.email, `delete ${cfg.slug} post`);
  const { error } = await (supabase.from(cfg.postsTable) as any).delete().eq("id", params.id);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
